...
def main():
    ...
    parser.add_argument(
        "--output_dir", default="js/", help="relative directory for output files"
    )
    parser.add_argument(
        "--key_file", default="keys.json", help="relative path to input keys file"
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
    ...
    for key, value in source_defs.items():
        if _NEWLINE_PATTERN.search(value):
            print(
                "ERROR: definition of {0} in {1} contained a newline character.".format(
                    key, args.source_lang_file
                )
            )
            sys.exit(1)
    ...
    for arg_file in args.files:
        ...
        if target_lang not in ("qqq", "keys", "synonyms", "constants"):
            ...
            if bad_keys:
                print(
                    "These keys in {0} contain non ascii characters: {1}".format(
            ...
            for key, value in target_defs.items():
                if _NEWLINE_PATTERN.search(value):
                    print(
                        ...
                        "a newline character.".format(key, arg_file)
                    )
...
""".format(
                ...
                if target_defs:
                    ...
                    if not args.quiet:
                        if extra_keys:
                            print(
                                "These extra keys appeared in {0}: {1}".format(
                                    filename, ", ".join(extra_keys)
                                )
                            )
                        if synonym_keys:
                            print(
                                "These synonym keys appeared in {0}: {1}".format(
