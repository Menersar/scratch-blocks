...
"""
    ...
    def format_js(self, code):
        ...
        def end_replacement(match):
            try:
                maybe_key = match.string[
                    match.string[: match.start()].rindex("{") + 1 : match.start()
                ]
            except ValueError:
                return "}}"
        ...
        return begin_brace.sub("{{", end_brace.sub(end_replacement, code)).format(
            **self.closure_env
        )
...
class Gen_compressed(threading.Thread):
    ...
    def gen_core(self, vertical):
        ...
        params = [
            (
                "compilation_level",
                "SIMPLE"
                if sys.argv.__len__() > 1 and sys.argv[1] == "production"
                else "SIMPLE",
            ),
        ...
        if "--debug" in sys.argv:
            params += [("debug", "true"), ("formatting", "PRETTY_PRINT")]

        # Read in all the source files.
        filenames = calcdeps.CalculateDependencies(
            search_paths, [os.path.join("core", "blockly.js")]
        )
    ...
    def do_compile_local(self, params, target_filename):
        ...
        filesizes = [
            os.path.getsize(value) for (arg, value) in params if arg == "js_file"
        ]
        return dict(
            compiledCode=stdout,
            statistics=dict(
    ...
    def report_errors(self, target_filename, filenames, json_data):
        ...
        if "serverErrors" in json_data:
            ...
            for error in errors:
                ...
                if error["file"]:
                    print(
                        "%s at line %d:" % (file_lookup(error["file"]), error["lineno"])
                    )
                    print(error["line"])
                    print((" " * error["charno"]) + "^")
                sys.exit(1)
...
class Gen_langfiles(threading.Thread):
    ...
    def _rebuild(self, srcs, dests):
        ...
        try:
            return max(os.path.getmtime(src) for src in srcs) > min(
                os.path.getmtime(dest) for dest in dests
            )
        except OSError as e:
    ...
    def run(self):
        ...
        if self._rebuild(
            [os.path.join("msg", "messages.js")],
            [
                os.path.join("msg", "json", f)
                for f in ["en.json", "qqq.json", "synonyms.json"]
            ],
        ):
            try:
        ...
        try:
            ...
            json_files = [
                ...
                if not (
                    file.endswith(
                        ("keys.json", "synonyms.json", "qqq.json", "constants.json")
                    )
                )
            ]
...
if __name__ == "__main__":
    try:
        ...
        calcdeps = import_path(
            os.path.join(closure_root, closure_library, "closure", "bin", "calcdeps.py")
        )

        # Sanity check the local compiler
        test_args = [closure_compiler, os.path.join("build", "test_input.js")]
        test_proc = subprocess.Popen(
            test_args, stdin=subprocess.PIPE, stdout=subprocess.PIPE
        )
